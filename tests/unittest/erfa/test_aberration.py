...
def test_aberration_direction(source):

    velocity = jnp.array([0, 0, 1e-8])
    solar_distance = 1.0
    lorenz_bm1 = np.sqrt(1.0 - (velocity ** 2).sum())

    p = aberration(source, velocity, solar_distance, lorenz_bm1)

    delta = p - source
    assert check_unitary(p)         # p should be a unit vector
...
def test_aberration_magnitude():
    ...
    for v, l in zip(velocity, lorenz_bm1):
        p = aberration(source, v, solar_distance, l)

        # calculate angular shifts along with x- and y-axes
        tx = np.atan2(p[0, 0], p[0, 2])
        ty = np.atan2(p[0, 1], p[0, 2])
