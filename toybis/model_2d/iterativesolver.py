...
def update_source_inner(src, att, cal,obs,sig_obs,priors,sig_prior,model,_min_nobs=10):
    """
    ...
    _iterate_att = vmap(model, (None, 0,None, 0), 0)
    ...
    _Jds = _Jacobian_autodiff(model,0,(None,0,None,0)) 
    ...
    c = jnp.hstack(_iterate_att(src[1:], exposures[:,2:], cal, exposures[:,1]))
    #create vector of observations to compare to
    o = observations[:, -1]
    
    #compute design matrix
    Ds = jnp.vstack(_Jds(src[1:], exposures[:,2:],cal,exposures[:,1]))
...
def update_attitude_inner(src, att,cal,obs,model,_min_nstars=100):
    ...
    _iterate_src = vmap(model, (0, None, None, None), 0)
    ...
    c = jnp.hstack(_iterate_src(sources[:,1:], att[2:],cal, att[1:]))
    #create vector of observations to compare to
    o = observations[:, -1]

    #compute design matrix
    De = jnp.vstack(_Jda(sources[:,1:],att[2:],cal,att[1:]))
